...
export async function getNotificationProviderConfig(): Promise<NotificationProviderConfig> {
  ...
  const config: NotificationProviderConfig = {
    ...
    smsAlertsEnabled: row?.smsAlertsEnabled ?? true,
    pushAlertsEnabled: row?.pushAlertsEnabled ?? true,
  };
...
export async function upsertNotificationSettings(
  ...
  return prisma.notificationSettings.upsert({
    ...
    create: {
      ...
      smsAlertsEnabled: input.smsAlertsEnabled ?? true,
      pushAlertsEnabled: input.pushAlertsEnabled ?? true,
      updatedById,
    },
    update: {
      emailTenantId,
