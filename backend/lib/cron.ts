...
import { sendEmailAlert, sendPushAlert, sendSmsAlert } from './notifications';
        ...
        if (overdueCheckIns.length === 0) {
          ...
          for (const checkIn of overdueCheckIns) {
            ...
            for (const recipient of recipients) {
              if (providerConfig.emailAlertsEnabled && recipient.adminEmail) {
              ...
              if (providerConfig.smsAlertsEnabled && recipient.adminPhone) {
                await sendSmsAlert(recipient.adminPhone, message, providerConfig);
              }
              if (providerConfig.pushAlertsEnabled) {
                for (const subscription of recipient.pushSubscriptions) {
                  const keepSubscription = await sendPushAlert(subscription, tier.emailSubject, message);
                  if (!keepSubscription) {
